...
class Beam():
    ...
    def advance(self, word_prob, word_length=None, inst_position=0, penalty = 0.04, init = None):
        "Update beam status and check if finished or not."
        # word_prob (beam_size, vocab_size) 
        num_words = word_prob.size(1)
        beam_size = word_prob.size(0)
        ...
        flat_beam_lk = beam_lk.view(-1) # 30522

        best_scores, best_scores_id = flat_beam_lk.topk(self.size, 0, True, True) # 1st sort (6)
